...
export default function AiRiskExplanation({ application }: AiRiskExplanationProps) {
  ...
  async function generateExplanation() {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || "AI 风险解释生成失败");
      }

      setExplanation(data.explanation);
    } catch (err) {
      setError(err instanceof Error ? err.message : "AI 风险解释生成失败");
    } finally {
      setLoading(false);
    }
  }
    ...
    <section className="rounded-md border border-line bg-panel p-5 shadow-glow">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h2 className="mt-2 text-xl font-semibold">基于 Zama 结果生成贷方说明</h2>
          <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-400">
            AI 只读取 Zama 合约输出的风险等级、抵押率、利率和还款金额，不读取收入、信用历史、负债压力等原始隐私数据。
          </p>
        </div>
        <button
